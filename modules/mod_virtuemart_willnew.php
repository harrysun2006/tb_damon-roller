...
*
* @version $Id: mod_virtuemart_featureprod.php,v 1.3.2.1 2006/05/06 10:05:27 soeren_nb Exp $
* @package VirtueMart
* @subpackage modules
*
...
if ( $category_id ) {
	...
	$q .= "AND #__{vm}_product.product_willnew='Y' \n";
	...
	while($db->next_record() ){
        ...
        ?>
			<tr align="center" class="<?php echo $sectioncolor ?>">
				<td align="left"> 
					<?php 
					$ps_product->show_snapshot22($db->f("product_sku"), $show_price, $show_addtocart);
					?>
				</td>
			</tr>
		<?php
		...
		elseif( $display_style== "table" ) {
			if( $i == 0 )
			echo "<tr>\n";
			echo "<td width=\"$width%\" align=\"center\" valign=\"bottom\">";
			$ps_product->show_snapshot($db->f("product_sku"), $show_price, $show_addtocart);
			echo "</td>\n";
			if ( ($i+1) % $products_per_row == 0)
			echo "</tr><tr><td colspan=\"".$products_per_row."\" height=\"10\"></td></tr><tr>\n";
			if( ($i+1) == $max_items )
			echo "</tr>\n";
		}
		$i++;
