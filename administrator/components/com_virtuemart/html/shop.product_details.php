...
require_once(CLASSPATH . 'class_pd.php' );
...
$mospage = new Mospage();
$mospageno = intval( mosgetparam($_REQUEST, "mospageno", null) );
if($mospageno<=0)$mospageno=1;
$url = $sess->url( $mm_action_url."index.php?page=shop.product_details&flypage=".$flypage."&product_id=". $product_id ."&category_id=". $category_id. "&manufacturer_id=" . $manufacturer_id);
$mospage->parse_text($product_description,$mospageno,$url);
$template = str_replace( "{product_main_text}", $mospage->main_text, $template );
$template = str_replace( "{product_nav_menu}", $mospage->nav_menu, $template );
