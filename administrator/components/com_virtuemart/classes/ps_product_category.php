...
class ps_product_category extends vmAbstractObject {
	...
	*/
	...
	function get_category_tree( $category_id=0,
		...
		$html = '<table class = "pc_nav_table"  border="0" cellspacing="0" cellpadding="0" >';
		/** FIRST STEP
		...
		**/
		...
		for($n = 0 ; $n < $nrows ; $n++) {
			...
			if( $allowed ) {
				...
				$catname =  $category_tmp[$row_list[$n]]["category_name"] ;

				$html .= '<tr><td class="'.$prifex. $class_sfx.'_'.$depth_list[$n].'">'
				. str_repeat("&nbsp;&nbsp;&nbsp;",$depth_list[$n]) 
        .'<a href="'. $sess->url(URL."index.php?page=shop.browse&amp;category_id=".$category_tmp[$row_list[$n]]["category_child_id"]) .'" title="'.$catname.'" >'
				. $catname
				. ps_product_category::products_in_category( $category_tmp[$row_list[$n]]["category_child_id"] )
				.'</a><img src="images/web/'.$prifex.$class_sfx.'_'.$depth_list[$n].'.gif"/></td></tr>';
			}
		}
		$html .= '</table>';
		return $html;
	...
	function get_child_list($category_id) {
		...
		if( $db->num_rows() > 0 ) {
			...
			while($db->next_record()) {
				...
				$html.= '<td align="center" width="'. $cellwidth .'%" valign="bottom"><br/>
