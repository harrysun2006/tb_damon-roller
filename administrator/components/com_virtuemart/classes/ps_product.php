...
class ps_product extends vmAbstractObject {
	...
	function update( &$d ) {
		...
		if (empty($d["product_new"])) $d["product_new"] = "N";
		if (empty($d["product_willnew"])) $d["product_willnew"] = "N";
		...
		$q .= "product_new='" . $d["product_new"] . "',";
		$q .= "product_willnew='" . $d["product_willnew"] . "',";
	...
	function show_image2($image, $args="", $resize=1, $path_appendix="product") {
		return $this->image_tag($image, $args, $resize, $path_appendix);
	}
	...
	***************************************************************************/
	...
	function product_snapshot( $product_sku, $show_price=true, $show_addtocart=true ) {
		...
		if ($db->next_record()) {
			...
			$html .= "<span class=\"product_name_snapshot\">".$db->f("product_name")."</span>\n";
			$html .= "<br />\n";
...
/**
	 ...
	 * @param boolean $show_addtocart Show the add-to-cart link?
	 */
	function show_snapshot22($product_sku, $show_price=true, $show_addtocart=true ) {

		echo $this->product_snapshot22($product_sku, $show_price, $show_addtocart);
	 ...
	 * @param boolean $show_addtocart Show the add-to-cart link?
	 */
	function product_snapshot22( $product_sku, $show_price=true, $show_addtocart=true ) {
		...
		if ($db->next_record()) {
			...
			$html .= "<table class=\"product_spec_d\">";
			$html .= "<tr>";
			$html .= "<td  rowspan=\"2\" height=\"112\">";
			$html .= "<a title=\"".$db->f("product_name")."\" href=\"". $sess->url($mm_action_url. "index.php" . $url)."\">";
			$html .= $this->image_tag($db->f("product_thumb_image"), "alt=\"".$db->f("product_name")."\"",0);
			$html .= "</a>";
			$html .= "</td>";
			...
			$html .= "<tr>";
			$html .= "<td valign=\"top\">";
			$html .= "<span class=\"product_s_desc_snapshot\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$db->f("product_s_desc")."</span>\n";
			if (_SHOW_PRICES == '1' && $show_price) {
				// Show price, but without "including X% tax"
				$html .= $this->show_price( $db->f("product_id"), true );
			}
