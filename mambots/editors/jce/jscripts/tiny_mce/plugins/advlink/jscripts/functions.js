...
function init() {
	...
	document.getElementById('searchframe').innerHTML = '<iframe src="index2.php?option=com_jce&no_html=1&task=plugin&plugin=advlink&file=search.php" name="search" id="search" title="Search" frameborder="0" width="280" height="115"></iframe>';
...
function buildOnClick() {
	...
	var pop_link = formObj.popupurl.value;
	pop_link = pop_link.replace( site_url+'/index.php', 'index2.php', 'gi' );
...
{
    var formObj = document.forms[0];
    var address = formObj.emailadd.value;
    if( !address.match( '@', 'g' ) )
    {
        ...
        var subj = formObj.emailsub.value;
        if( subj != '' ) subj = '?subject='+subj;

        var email_address = address+subj;
        formObj.href.value = email_address;
    }
}
