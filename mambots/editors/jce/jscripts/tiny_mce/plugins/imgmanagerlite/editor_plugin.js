tinyMCE.importPluginLanguagePack('imgmanagerlite', tinyMCE.getParam('lang_list'));

var TinyMCE_ImageManagerLitePlugin = {
	getInfo : function() {
		return {
			longname : 'JCE Image Manager Lite',
			author : 'Ryan Demmer',
	...
	getControlHTML : function(cn) {
		switch (cn) {
			case "imgmanagerlite":
				return tinyMCE.getButtonHTML(cn, 'lang_imgmanagerlite_desc', '{$pluginurl}/images/imgmanagerlite.gif', 'mceImageManagerLite');
		}
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			case "mceImageManagerLite":
				var template = new Array();

				template['file']   = tinyMCE.getParam('site')+'/index2.php?option=com_jce&no_html=1&task=plugin&plugin=imgmanagerlite&file=manager.php';
                template['width']  = 560;
			    template['height'] = 560;
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && tinyMCE.getAttrib(node, 'class').indexOf('mceItem') == -1) {
				tinyMCE.switchClass(editor_id + '_imgmanagerlite', 'mceButtonSelected');
				return true;
			}
		} while ((node = node.parentNode));

		tinyMCE.switchClass(editor_id + '_imgmanagerlite', 'mceButtonNormal');
...
tinyMCE.addPlugin("imgmanagerlite", TinyMCE_ImageManagerLitePlugin);
